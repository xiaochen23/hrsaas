...
if (isProd) {
  ...
  cdn = {
    // 提前引入elementUI样式 放置css文件目录
    css: ['https://unpkg.com/element-ui/lib/theme-chalk/index.css'],
    // 放置js文件目录  vue要放在第一个
    js: ['https://unpkg.com/vue/dist/vue.js', // vuejs
...
module.exports = {
  ...
  configureWebpack: {
    ...
    // 要排除的包名
    // key(是要排除的包): value(是实际上引入的包的全局的变量名)
  ...
  chainWebpack(config) {
    ...
    config.plugin('html').tap((args) => {
      // args 是注入html模板的一个变量
      args[0].cdn = cdn // 后面这个cdn是上面定义的变量
      return args // 需要返回args这个参数
    })
